# Protobuf-Dreamer
A tiled DeepDream project for creating any size of image, on both CPU and GPU. Tensorflow should be compiled for either the CPU, or GPU depending on what your prefer. The CPU is slower, but this project should allow anyone to create an image of any size. The tiling code is based on [jnordberg](https://github.com/jnordberg)'s [DreamCanvas](https://github.com/jnordberg/dreamcanvas) project. This project should run on any hardware that supports Tensorflow (Including micro computers like the Raspberry Pi), but obviously better hardware will be able to create larger images, faster.
